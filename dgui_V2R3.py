...
from PyQt5.QtCore    import QSize, Qt, QRect, QPoint, pyqtSignal
...
from PyQt5.QtWidgets import (QApplication, QCheckBox, QGroupBox,
        ...
        QAction, QShortcut, QToolBar, QMainWindow, QDialog,
...
###################################################
    def close_dgui(self,scope):
        ...
        # there is no INPUTFILEPATH in the .ini file, so append it             
            with open(dgpath + os.sep + "dgui.ini","a") as fp:  
                fp.write('\n'+'INPUTFILEPATH ' + last_ifpath + '\n')
        else:        
...
# Open the DYNAC User Guide (assumes it is in ..../dynac/help directory)
        if (platform.system() == 'Windows') :
            path_to_acro = "C:\Program Files\Adobe\Acrobat DC\Acrobat\Acrobat.exe"
...
#                cmd='"C:\Program Files\Adobe\Acrobat DC\Acrobat\Acrobat.exe"' + ' /n ' +  default_ugpath + os.sep + 'dynac_UG.pdf'
                cmd = '"C:\Program Files\Adobe\Acrobat DC\Acrobat\Acrobat.exe"' 
                argmnt1 =  ' /n '  
...
# DYNAC GUI User Guide is assumed to be in ..../dynac/help directory
        ...
        txt2= dguiv + " requires DYNAC V7R1 or newer and python3.7, 3.8 or 3.9"
        abouttxt = txt1 + txt2
...
#       get the project number
            ...
            self.log_it(mytext)
            choice = QtWidgets.QMessageBox.question(self, 'Project Directory',"Change the project directory?",QtWidgets.QMessageBox.Yes | QtWidgets.QMessageBox.No)
            if choice == QtWidgets.QMessageBox.Yes:
                choice2 = QtWidgets.QMessageBox.question(self, 'Project Directory',"Create a new project directory?",QtWidgets.QMessageBox.Yes | QtWidgets.QMessageBox.No)
...
######################################################
        ...
        if pfname:
            try:
                ...
                num_args = len(first_row.split())
                # read the remainder of the data
                myDataFrame = pd.read_csv(pfname, skiprows=1, sep='\s+', header=None, names=["ELEMENT", "l(m)", "x(mm)", "y(mm)", "z(deg)", "z(mm)", "Ex,n,RMS(mm.mrd)", "Ey,n,RMS(mm.mrd)", "Ez,RMS(keV.ns)", "Wcog(MeV)", "#particles", "xmin(mm)", "xmax(mm)", "ymin(mm)", "ymax(mm)", "tmin(s)", "tmax(s)", "phmin(deg)", "phmax(deg)", "Wmin(MeV)", "Wmax(MeV)", "Dx(m)", "Dy(m)", "dW(MeV)", "Wref(MeV)", "Tref(s)", "Tcog(s)", "xbar(mm)", "ybar(mm)"])
