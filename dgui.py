...
dgparser.add_argument('-p',help='here P is the path to the location of dgui.py; no space between -p and the path! -pP is mandatory if none of the other optional arguments are used', action='store', nargs=1)
...
#        print("Using data from 2: ",dmpfname)
        if pfname:
            try:
                ...
                dmpDataFrame = pd.read_csv(dmpfname, skiprows=3, sep='\s+', header=None, comment='#',names=["element", "Z", "trans", "PHIs", "TOF(COG)", "Beta(COG)", "Wcog", "TOF(REF)", "Beta(REF)", "Wref", "Ex,RMS,n", "Ey,RMS,n", "El,RMS", "dWref", "EffVolt"])
